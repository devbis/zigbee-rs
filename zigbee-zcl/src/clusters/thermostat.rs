...
impl ThermostatCluster {
    pub fn new() -> Self {
        ...
        let _ = store.register(
            AttributeDefinition { id: ATTR_LOCAL_TEMPERATURE, data_type: ZclDataType::I16, access: AttributeAccess::Reportable, name: "LocalTemperature" },
            ZclValue::I16(2200), // 22.00 deg C
        );
        let _ = store.register(
            AttributeDefinition { id: ATTR_OUTDOOR_TEMPERATURE, data_type: ZclDataType::I16, access: AttributeAccess::ReadOnly, name: "OutdoorTemperature" },
            ZclValue::I16(0),
        );
        let _ = store.register(
            AttributeDefinition { id: ATTR_OCCUPANCY, data_type: ZclDataType::U8, access: AttributeAccess::ReadOnly, name: "Occupancy" },
            ZclValue::U8(0x01), // Occupied
        );
        let _ = store.register(
            AttributeDefinition { id: ATTR_ABS_MIN_HEAT_SETPOINT_LIMIT, data_type: ZclDataType::I16, access: AttributeAccess::ReadOnly, name: "AbsMinHeatSetpointLimit" },
            ZclValue::I16(700), // 7.00 deg C
        );
        let _ = store.register(
            AttributeDefinition { id: ATTR_ABS_MAX_HEAT_SETPOINT_LIMIT, data_type: ZclDataType::I16, access: AttributeAccess::ReadOnly, name: "AbsMaxHeatSetpointLimit" },
            ZclValue::I16(3000), // 30.00 deg C
        );
        let _ = store.register(
            AttributeDefinition { id: ATTR_ABS_MIN_COOL_SETPOINT_LIMIT, data_type: ZclDataType::I16, access: AttributeAccess::ReadOnly, name: "AbsMinCoolSetpointLimit" },
            ZclValue::I16(1600), // 16.00 deg C
        );
        let _ = store.register(
            AttributeDefinition { id: ATTR_ABS_MAX_COOL_SETPOINT_LIMIT, data_type: ZclDataType::I16, access: AttributeAccess::ReadOnly, name: "AbsMaxCoolSetpointLimit" },
            ZclValue::I16(3200), // 32.00 deg C
        );
        let _ = store.register(
            AttributeDefinition { id: ATTR_OCCUPIED_COOLING_SETPOINT, data_type: ZclDataType::I16, access: AttributeAccess::ReadWrite, name: "OccupiedCoolingSetpoint" },
            ZclValue::I16(2600), // 26.00 deg C
        );
        let _ = store.register(
            AttributeDefinition { id: ATTR_OCCUPIED_HEATING_SETPOINT, data_type: ZclDataType::I16, access: AttributeAccess::ReadWrite, name: "OccupiedHeatingSetpoint" },
            ZclValue::I16(2000), // 20.00 deg C
        );
        let _ = store.register(
            AttributeDefinition { id: ATTR_MIN_HEAT_SETPOINT_LIMIT, data_type: ZclDataType::I16, access: AttributeAccess::ReadWrite, name: "MinHeatSetpointLimit" },
            ZclValue::I16(700),
        );
        let _ = store.register(
            AttributeDefinition { id: ATTR_MAX_HEAT_SETPOINT_LIMIT, data_type: ZclDataType::I16, access: AttributeAccess::ReadWrite, name: "MaxHeatSetpointLimit" },
            ZclValue::I16(3000),
        );
        let _ = store.register(
            AttributeDefinition { id: ATTR_MIN_COOL_SETPOINT_LIMIT, data_type: ZclDataType::I16, access: AttributeAccess::ReadWrite, name: "MinCoolSetpointLimit" },
            ZclValue::I16(1600),
        );
        let _ = store.register(
            AttributeDefinition { id: ATTR_MAX_COOL_SETPOINT_LIMIT, data_type: ZclDataType::I16, access: AttributeAccess::ReadWrite, name: "MaxCoolSetpointLimit" },
            ZclValue::I16(3200),
        );
        let _ = store.register(
            AttributeDefinition { id: ATTR_CONTROL_SEQUENCE_OF_OPERATION, data_type: ZclDataType::Enum8, access: AttributeAccess::ReadWrite, name: "ControlSequenceOfOperation" },
            ZclValue::Enum8(0x04), // Cooling and heating
        );
        let _ = store.register(
            AttributeDefinition { id: ATTR_SYSTEM_MODE, data_type: ZclDataType::Enum8, access: AttributeAccess::ReadWrite, name: "SystemMode" },
            ZclValue::Enum8(SYSTEM_MODE_AUTO),
        );
        let _ = store.register(
            AttributeDefinition { id: ATTR_THERMOSTAT_RUNNING_MODE, data_type: ZclDataType::Enum8, access: AttributeAccess::ReadOnly, name: "ThermostatRunningMode" },
    ...
    pub fn set_local_temperature(&mut self, hundredths: i16) {
        let _ = self.store.set_raw(ATTR_LOCAL_TEMPERATURE, ZclValue::I16(hundredths));
    }
}

impl Cluster for ThermostatCluster {
    fn cluster_id(&self) -> ClusterId { ClusterId::THERMOSTAT }
    ...
    fn handle_command(
        ...
        match cmd_id {
            CMD_SETPOINT_RAISE_LOWER => {
                ...
                if mode == 0 || mode == 2 {
                    if let Some(ZclValue::I16(cur)) = self.store.get(ATTR_OCCUPIED_HEATING_SETPOINT) {
                        let new_val = cur.saturating_add(amount);
                        let _ = self.store.set(ATTR_OCCUPIED_HEATING_SETPOINT, ZclValue::I16(new_val));
                    }
                }
                if mode == 1 || mode == 2 {
                    if let Some(ZclValue::I16(cur)) = self.store.get(ATTR_OCCUPIED_COOLING_SETPOINT) {
                        let new_val = cur.saturating_add(amount);
                        let _ = self.store.set(ATTR_OCCUPIED_COOLING_SETPOINT, ZclValue::I16(new_val));
                    }
                }
                Ok(heapless::Vec::new())
            }
