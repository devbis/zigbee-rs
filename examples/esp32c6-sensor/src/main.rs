...
async fn main(_spawner: Spawner) {
    ...
    //
    // The `zigbee-mac` crate's "esp32c6" feature wraps `esp-radio`'s
    // IEEE 802.15.4 peripheral through the `MacDriver` trait.
    // -----------------------------------------------------------------------
    let ieee802154 = esp_radio::ieee802154::Ieee802154::new(
    ...
    loop {
        ...
        match event_loop::stack_tick(&mut device).await {
            ...
            TickResult::Event(_event) => {
                esp_println::println!("[zigbee] Stack event received");
            }
        ...
        if joined {
            ...
            match sht.measure(sht3x::Repeatability::High, &delay) {
                Ok(measurement) => {
